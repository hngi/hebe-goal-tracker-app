...
const Goal = require('..contrllers/goals');

const requireToken = jsonwebtoken.authenticate('bearer', { session: false });
...
router.post('/sign-in', (req, res, next) => {
  const passW = req.body.credentials.password;
  let user;
  ...
  User.findOne({ email: req.body.credentials.email })
    .then(record => {
      ...
      return bcrypt.compare(passW, user.hashedPassword)
    })
    .then(correctPassword => {
      // if the passwords matched
      if (correctPassword) {
        // the token will be a 16 byte random hex string
        const token = jsonwebtoken.randomBytes(16).toString('hex');
        user.token = token;
...
router.delete('/sign-out', requireToken, (req, res, next) => {
  ...
  req.user.token = jsonwebtoken.randomBytes(16)
